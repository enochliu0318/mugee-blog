...
**首次访问不高亮，但新文章会亮。** 判定条件其实是"没见过 || 内容变了"：浏览器第一次到访列表页时，会静默把当前所有文章记为基线——整站对第一次来的读者都是新的，逐篇提示只是噪音；此后列表里再出现"没有基线"的文章，就一定是新发布的，它和"指纹变了"的旧文章一样亮灯，悬停提示也会区分"新文章"和"已更新"。
